...
Related cases: app_ui, app_menu, settings, settings-defaults.
...
- A newly chosen UI mode takes effect when Settings is applied, on every open window at once, with no restart.
  Selecting it does not preview it.
...
- The appearance covers the preview, editor, and settings screens, including the preview's menu. The tray
  icon's context menu, the region selection overlay, and message boxes are drawn by Windows and are not
  affected.
