...
/// <summary>The settings as they are kept between application runs.</summary>
public readonly record struct StoredSettings(Hotkey Hotkey, CaptureMode CaptureMode)
{
    /// <summary>The settings used when none are configured.</summary>
    public static StoredSettings Default { get; } = new(Hotkey.Capture, CaptureMode.FullScreen);
}
